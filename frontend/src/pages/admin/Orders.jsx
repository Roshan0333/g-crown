...
const ALL_STATUSES = ["All", "Confirmed", "Accepted", "Shipped", "Delivered", "Cancelled","Returned","Refunded"];
...
const Orders = () => {
  ...
  const filteredOrders = useMemo(() => {
    let data = [...orders];
if (filterStatus !== "All") data = data.filter(o => o.orderStatus === filterStatus);
...
o.displayOrderId.includes(searchQuery)
  ...
  const stats = useMemo(() => {
  const statusCounts = {};
  ALL_STATUSES.slice(1).forEach(status => 
    ...
    <div className="p-6 bg-gray-100 min-h-screen space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-xl shadow">
        <div className="flex items-center gap-2">
          ...
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="p-2 border rounded-lg outline-none focus:ring-2 focus:ring-indigo-500">
